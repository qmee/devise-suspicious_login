Warden::Manager.after_set_user except: :fetch do |user, warden, opts|
  if warden.winning_strategy.class == Devise::Strategies::DatabaseAuthenticatable && user && !user.token_login? && user.respond_to?(:suspicious?) && user.suspicious?(warden.request)
