...
class SuspiciousLoginIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test 'normal login' do
    assert_equal(@user.suspicious_login?, false)
  end

  test 'dormant login' do
    assert_equal(@dormant_user.suspicious_login?, true)
  end

  test 'suspicious login' do
    assert_equal(@external_suspicious_user.suspicious_login? { true }, true)
  end
end